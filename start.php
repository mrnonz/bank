						...
						<div class="bank-detail">
							<div class="encode-wrap">
								...
								<form action="" method="post">
									...
									<div>
										<label>เลขที่บัญชีธนาคาร (ใส่ตามรูปแบบ 123-4-56789-0)</label>
									...
									<hr />
									<div class="title"><img src="images/krungsri.png" width="40" /> ธนาคารกรุงศรีอยุธยา</div>
									<div>
										<label>Username</label>
										<input type="text" name="ks_username" class="span4">
									</div>
									<div>
										<label>Password</label>
										<input type="password" name="ks_password" class="span4">
									</div>
									<div>
										<label>เลขที่บัญชีธนาคาร (ใส่ตามรูปแบบ 123-4-56789-0)</label>
										<input type="text" name="ks_account_name" placeholder="123-4-56789-0" class="span4">
									</div>
								...
								if (!empty($_POST['encode_key'])) {
									...
									if (!(empty($_POST['ks_username']) || empty($_POST['ks_password']) || empty($_POST['ks_account_name']))) {
										$string = $_POST['ks_username'];
										$encrypted = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($key), $string, MCRYPT_MODE_CBC, md5(md5($key))));
										$text .= '$KS_USERNAME = \''.$encrypted.'\';'.PHP_EOL;

										$string = $_POST['ks_password'];
										$encrypted = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($key), $string, MCRYPT_MODE_CBC, md5(md5($key))));
										$text .= '$KS_PASSWORD = \''.$encrypted.'\';'.PHP_EOL;

										$text .= '$KS_ACCOUNT_NAME = \''.$_POST['ks_account_name'].'\';'.PHP_EOL.PHP_EOL;
									}
